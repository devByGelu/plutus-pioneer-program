...
mkValidator dat () ctx = ( validSig ( beneficiary1 dat ) && beforeOrAtDeadline ) || -- if either beneficiary1 has signed the transaction and the current slot is before or at the deadline
                         ( validSig ( beneficiary2 dat) && afterDeadline ) -- if beneficiary2 has signed the transaction and the deadline has passed.
    where
        info :: TxInfo
        info = scriptContextTxInfo ctx

        validSig :: PubKeyHash -> Bool 
        validSig pkh = txSignedBy info pkh

        range :: POSIXTimeRange
        range = txInfoValidRange info

        beforeOrAtDeadline :: Bool 
        beforeOrAtDeadline = contains (to ( deadline dat )) range

        afterDeadline :: Bool 
        afterDeadline = contains (from ( deadline dat + 1 )) range -- Add 1 to exclude the deadline
